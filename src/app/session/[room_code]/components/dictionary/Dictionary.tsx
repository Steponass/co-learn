...
export default function Dictionary() {
  ...
  const searchWord = async (e: React.FormEvent) => {
    ...
    if (!input.trim() || !apiKey) {
      setError(apiKey ? "Please enter a word to search" : "API key not configured");
      return;
    }
    ...
    try {
      const response = await fetch(
        `https://www.dictionaryapi.com/api/v3/references/learners/json/${encodeURIComponent(input.trim())}?key=${apiKey}`
      );
      ...
      if (Array.isArray(data) && data.length > 0 && typeof data[0] === 'object') {
          ...
          entry.def.forEach(defGroup => {
            defGroup.sseq.forEach(senseSeq => {
              senseSeq.forEach(sense => {
                if (Array.isArray(sense) && sense.length > 1 && typeof sense[1] === 'object') {
                  const senseObj = sense[1] as { dt?: Array<Array<string>> };
                  if (senseObj.dt) {
                    senseObj.dt.forEach(dtItem => {
                      if (dtItem[0] === 'text' && dtItem[1]) {
                        const cleanDef = stripHtml(dtItem[1]).replace(/^\{bc\}/, '').trim();
                        if (cleanDef) {
        ...
        setDictionaryData({
          ...
          definitions: definitions.length > 0 ? definitions : ['No definition available'],
          audioUrl,
        });
      } else {
  ...
  return (
    <div className={classes.dictionary_wrapper}>
    <div className="stack">
      <h5>Dictionary</h5>
